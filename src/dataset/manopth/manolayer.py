...
class ManoLayer(Module):
    ...
    def forward(self,
        ...
        th_jtr = th_jtr + th_trans[:, None]
        th_verts = th_verts + th_trans[:, None]

        root_trans = th_jtr[:, 0:1]
        
        th_verts = torch.matmul(root_rot.transpose(1, 2), (th_verts - root_trans).transpose(1, 2)).transpose(1, 2)
        th_jtr = torch.matmul(root_rot.transpose(1, 2), (th_jtr - root_trans).transpose(1, 2)).transpose(1, 2)
