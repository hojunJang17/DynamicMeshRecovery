...
def compute_rotation_matrix_from_2d(param):  # parameter range: -inf ~ inf
    # param  : #(B, 2)

    B, C = param.size()
    assert C == 2

    param += 1e-3   # (to avoid nans)

    x = normalize_vector(param)
    cos, sin = torch.chunk(x, 2, dim=-1)
    y = torch.cat([-sin, cos], dim=-1)
    matrix = torch.stack([x, y], dim=1)  # (B, 2, 2)
...
def lerp(one, two, a):
    return one * (1 - a) + two * a

def quaternion_ln(quat):
    a = quat[..., 0:1]
    b = quat[..., 1]
    c = quat[..., 2]
    d = quat[..., 3]
    
    first = torch.log(torch.norm(quat, dim=-1, keepdim=True))
    vec_norm = torch.norm(quat[..., 1:], dim=-1, keepdim=True)
    three = quat[..., 1:] / vec_norm * torch.acos(a / torch.norm(quat, dim=-1, keepdim=True))
    return torch.cat([first, three], dim=-1)

def quaternion_exp(quat):
    a = quat[..., 0:1]
    b = quat[..., 1]
    c = quat[..., 2]
    d = quat[..., 3]
    ...
    vec_norm = torch.norm(quat[..., 1:], dim=-1, keepdim=True)
    first = torch.exp(a) * torch.cos(vec_norm)
    three = quat[..., 1:] / vec_norm * torch.sin(vec_norm)

    return torch.cat([first, three], dim=-1)


def quaternion_pow(quat, n):
    return quaternion_exp(quaternion_ln(standardize_quaternion(quat)) * n)

def slerp(one, two, a):
    one_inv = quaternion_invert(one)
    temp_term = quaternion_multiply(one_inv, two)
    temp_term = quaternion_pow(temp_term, a)
    temp_term[torch.isnan(temp_term)] = 0
    
    result = quaternion_multiply(one, temp_term)
    return result

def acos_safe(input, eps=1e-6):
    return torch.acos(torch.clamp(input, -1 + eps, 1 - eps))

def rotation_6d_difference(r1, r2):
    """
    r1 : gt (B, T, K * 6)
    r2 : estimate (B, T, K * 6)
    """
    B = r1.shape[0]
    T = r1.shape[1]
    r1_mat = rotation_6d_to_matrix(r1.view(B, T, -1, 6))
    r2_mat = rotation_6d_to_matrix(r2.view(B, T, -1, 6))

    r2_inv = torch.inverse(r2_mat)

    mult_result = torch.einsum('...ij,...jk->...ik', r1_mat, r2_inv)
    trace_result = torch.einsum('...ii', mult_result)
    
    return acos_safe((trace_result - 1) / 2.)  # (B, T, K)